...
export const TrustSection: React.FC = () => {
  return (
    <section className="relative w-full bg-[var(--bg-primary)] py-16 sm:py-24 md:py-28 border-t border-[var(--border-color)] overflow-hidden transition-colors duration-300">
      <div className="w-full max-w-[1800px] mx-auto px-5 sm:px-8 md:px-12 mb-10 sm:mb-16">
        <div className="flex flex-col md:flex-row md:items-end justify-between">
          <div>
            <h2 className="font-headline font-bold text-[var(--text-primary)] text-[clamp(1.35rem,4.5vw,2rem)] md:text-[clamp(2.5rem,5vw,4rem)] leading-[1.05] md:leading-[0.9] tracking-[-0.03em]">
      ...
      <div className="marquee-parent-container relative w-full overflow-hidden py-4 select-none">
        ...
        <div className="hidden md:block absolute left-0 top-0 bottom-0 w-32 bg-gradient-to-r from-[var(--bg-primary)] to-transparent z-10 pointer-events-none" />
        <div className="hidden md:block absolute right-0 top-0 bottom-0 w-32 bg-gradient-to-l from-[var(--bg-primary)] to-transparent z-10 pointer-events-none" />
        ...
        <div className="flex w-max">
          ...
          <div className="flex shrink-0 items-start gap-[60px] animate-infinite-scroll pr-[60px]">
            {REVIEWS.map((review, idx) => (
              ...
              >
                <blockquote className="font-headline font-bold text-[var(--text-primary)] text-[clamp(1rem,2.2vw,1.45rem)] md:text-[clamp(1.15rem,2.5vw,1.55rem)] leading-[1.3] tracking-tight mb-3">
                  "{review.quote}"
                </blockquote>
                <cite className="not-italic font-sans text-[11px] sm:text-[13px] md:text-sm text-[var(--text-muted)] font-medium tracking-normal">
                  — {review.author}
                </cite>
              </div>
            ))}
