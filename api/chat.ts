...
// Instantiate the GenAI SDK with Vercel Environment Variable
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    // Retain only the last 8 messages to manage context memory and token cost
    const conversationHistory = messages.slice(-8);
    const userMessage = conversationHistory[conversationHistory.length - 1]?.content || '';

    // Verify environment key is present; otherwise triggers automated fallback
    if (!process.env.GEMINI_API_KEY) {
      throw new Error('GEMINI_API_KEY environment variable is not defined');
    }

    // Format the incoming messages into content structures required by GenAI SDK
    ...
    const replyText = response.text || getLocalFallbackReply(userMessage);
    ...
    // Return structured, helpful local keyword fallback
