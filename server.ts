    ...
    if (netlifyUrl) {
      try {
        const netlifyRes = await fetch(netlifyUrl, {
          ...
          body: JSON.stringify({ messages: conversationHistory, message: userMessage }),
        });
        if (netlifyRes.ok) {
          const data = await netlifyRes.json();
          res.json({ reply: data.reply || data.response || data.message || getLocalFallbackReply(userMessage) });
          return;
        }
      } catch (err) {
